<div id="$id" class="RequestHandler FormField DataObjectManager $NestedType field" href="$CurrentLink">
	...
	<div class="<% if HasFilter %>filter<% end_if %>">
		<% if Can(add) %>	
			<a class="popup-button btn btn-primary pull-right" rel="$PopupWidth" href="$AddLink" alt="add">
				<i class="icon-plus icon-white"></i> <% sprintf(_t('DataObjectManager.ADDITEM','Add %s',PR_MEDIUM,'Add [name]'),$AddTitle) %>
			</a>	
			<h3>$PluralTitle</h3>
		<% else %>
		  <h3>$PluralTitle</h3>
