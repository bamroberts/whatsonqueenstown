...
function require(fieldName,cachedError) {
	...
	if(el != null) {
		...
		// if its a hidden field we are possibly dealing with a bootstrap field
		if (baseEl.type === 'hidden') {
			if (jQuery) {
				$base_el = jQuery(baseEl);
				$el_container = $base_el.closest('.bootstrapselectiongroup');
				if ($el_container.length > 0) {
					el = $el_container.get(0);
					baseEl = $el_container.find('div').get(0);
				}
			}
		}
