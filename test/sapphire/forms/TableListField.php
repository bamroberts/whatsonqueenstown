...
JS
	...
	function Headings() {
		...
		foreach($this->fieldList as $fieldName => $fieldTitle) {
			...
			$headings[] = new ArrayData(array(
				"Name" => $fieldName, 
				//Performance: Don't do a massive DB lookup through all ancestors for table list field headings
				//Just use the summary_fields set for the object in the tablelist field
				//"Title" => ($this->sourceClass) ? singleton($this->sourceClass)->fieldLabel($fieldTitle) : $fieldTitle,
				//Ensure there are no 0 indexed elements in your summary_fields for the model
				//Otherwise the DataObject::sumaryFields() function will reindex EVERYTHING
				"Title" => $fieldTitle,
	...
	function NextLink() {
		...
		$baseLink = ($this->paginationBaseLink) ? $this->paginationBaseLink : $this->Link();
//      echo 'Pagin: ' . $this->paginationBaseLink . '  Baselink: ' . $baseLink; exit;
		$link = Controller::join_links($baseLink, "?ctf[{$this->Name()}][start]={$start}");
		if($this->extraLinkParams) $link .= "&" . http_build_query($this->extraLinkParams);
