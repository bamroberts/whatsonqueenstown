...
class SS_Datetime extends Date {
	...
	function setValue($value) {
		...
		if(is_array($value) && isset($value['date'])) {
			if(preg_match('#^([0-9]+)/([0-9]+)/([0-9]+)$#', $value['date'], $parts)) {
				$valueDate = "$parts[2]/$parts[1]/$parts[3]";
			}
			if (!isset($valueDate)) $valueDate = date('Y-m-d', time());
			$valueTime = isset($value['time']) ? $value['time'] : date('H:i:s', time());
			$value = date('Y-m-d', strtotime($valueDate)).' '.date('H:i:s', strtotime($valueTime));
		}
