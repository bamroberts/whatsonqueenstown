...
class ViewableData extends Object implements IteratorAggregate {
	...
	public function renderWith($template, $customFields = null) {
		if(!is_object($template)) {
			...
			//@FirstCore change - Needed to make sure includes are themed 
			$template = new SSViewer($template);
		}
	...
	public function cachedCall($field, $arguments = null, $identifier = null) {
		//Method name is identifier if none deligated
		if(!$identifier) {
			$identifier = $field;
		}
		
		//Cache for unique methods
		if($arguments) {
			foreach($arguments as $argument){
				if($argument instanceof DataObject) {
					$identifier .= "_{$argument}:{$argument->ID}";
				} elseif ($argument instanceof DataObjectSet) {
					$byClassName = $argument->GroupBy('ClassName');
					foreach($byClassName as $class=>$records) {
						if(!$class) {continue;}
						$identifier .= "_{$class}s:".join(',', $records->Map('ID','ID'));
					}
				} elseif (!is_object($argument)) {
					$identifier .= "_{$argument}";
				}
			}
		}
